...
public class ForgetPWDFragment extends StepFragment {
    ...
    public static final String MARK ="ForgetPWDFragment";
    @Override
    ...
    protected View createContent(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (v == null) {
            v = inflater.inflate(R.layout.fragment_forget_pwd, null);
        }
        return v;
    }
    ...
    private void doGetPwd(){
        ...
        LoginPageManager.getInstance().doLoginPost(tel,pwd,captcha,MARK);
    }
    @Override
    public void free() {
    ...
    public void onEventMainThread(LoginPageEvents event) {
        if (event!=null&&MARK.equals(event.getMark())) {
            cancelOprationDialogFragment();
            Toast.makeText(getStepActivity(), event!=null?event.getMessage():"找回密码失败", Toast.LENGTH_SHORT).show();
